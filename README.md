...
6. 저장된 `tempArr` 내부요소들은 이동방향에 알맞게 오른쪽 혹은 왼쪽으로 3칸씩 밀어준다. (3칸을 1면으로 치부)<br>

- 방향에 `'`가 있을 경우 왼쪽으로, 없을 경우 오른쪽으로 이동시킨다.<br>
...
9. 새로운 `div`에 template literal로 `triple_arr`데이터를 입력해 매회전마다 UI에 렌더링한다.<br>
